...
public class ProcessStudentInfo {
	...
	public static void main(String[] args) throws ParserConfigurationException, SAXException, IOException {
		...
		list.put("Stu",seleniumStudents);

		/*for (Student student : seleniumStudents) {
			if (list.containsKey(student.score)) {
				list.get(student.score).add(student.toString());
			} else {
				List<String> studentResults = new LinkedList<String>() {
				};
				studentResults.add(student.toString());
				list.put(student.score, studentResults);
			}
		}*/
		...
		for (String key:list.keySet()) {
			for (Student student: list.get(key)) {
